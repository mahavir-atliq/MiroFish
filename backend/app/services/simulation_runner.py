...
class SimulationRunState:
    ...
    # Recent actions (for real-time front-end display)
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        # If a max rounds limit was specified, truncate
        ...
        # If graph memory updates are enabled, create the updater
        ...
        try:
            # Build the run command with full path
            # New log structure:
            ...
            # If a max rounds limit was specified, add it to the command-line arguments
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # Create the main log file to prevent process blocking from full stdout/stderr pipe buffers
            ...
            # Set the working directory to the simulation directory (database and other files will be created here)
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create a new process group, ensuring all related processes can be terminated on server shutdown
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # After the process ends, do a final log read
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round information (from round_end events)
                                    ...
                                    # Update the independent round and time for each platform
                                    if platform == "twitter":
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Platforms are considered enabled if their corresponding actions.jsonl file exists.
        ...
        # Check which platforms are enabled (determined by whether their files exist)
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = force termination, /T = terminate the process tree (including child processes)
            ...
            try:
                ...
                # If the process has not exited after the timeout, force-send SIGKILL
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer the record's own platform field, otherwise use the default platform
    ...
    def get_all_actions(
        ...
        # Read Twitter action file (platform is automatically set to "twitter" based on the file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action file (platform is automatically set to "reddit" based on the file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # If per-platform files are absent, fall back to the legacy single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # The legacy format file should already contain the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
